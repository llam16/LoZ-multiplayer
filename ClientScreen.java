...
public class ClientScreen extends JPanel implements KeyListener {
	...
	public void checkMap(String dir) {
		if (game.getItemMap().get(game.getP2().getLocation()) == null) {
    			System.out.println("empty");
    		}
        	else if ((int) (game.getItemMap().get(game.getP2().getLocation())) == 1) {
        		game.getP2().collectedItem();
						//Sound.playSound("LOZ_Get_Rupee.wav");
						//Sound.stopSound("LOZ_Get_Rupee.wav");
						game.checkWinner();
        		game.getItemMap().put(game.getP2().getLocation(), 0);
        	}
			else if ((int) (game.getItemMap().get(game.getP2().getLocation())) == 2) {
        		game.getP2().loseHealth();
						//Sound.playSound("LOZ_Link_Hurt.wav");
						//Sound.stopSound("LOZ_Link_Hurt.wav");
						game.getItemMap().remove(game.getP2().getLocation());
						if (game.getP2().getHealthSize() == 0) {
							game.setWinner(1);
						}
						else {
	        		game.getP2().setLocation(new Location(4,5));
						}
        	}
			else if ((int) (game.getItemMap().get(game.getP2().getLocation())) == 3 || (int) (game.getItemMap().get(game.getP2().getLocation())) == 4 || (int) (game.getItemMap().get(game.getP2().getLocation())) == 5) {
        		if (dir.equals("up")) {
					game.getP2().down();
				}
    ...
    g.drawString("PLAYER 2", 500, 60);
    g.setColor(Color.white);
    g.setFont(new Font("Helvetica", Font.BOLD, 50));
    g.drawString("PLAYER 2", 503, 60);
	...
	g.drawString(" x " + game.getP2().getNumItems(), 575, 232);
	...
	for (int i = 0; i < game.getP2().getHealthSize(); i++) {
	...
	for (int i = 0; i < (3 - game.getP2().getHealthSize()); i++) {
		g.drawImage(img, xPos, yPos, 30, 30, null);
		xPos += 30;
	}
